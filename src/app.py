...
def create_command_msg(title, desc, name, icon_url):
...
class BotClient(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith(command_prefix+"start"):
            ...
            if not isStarted:
                desc = "翻訳開始！"
                embed = create_command_msg("start", desc, botName, botIcon)
        ...
        if message.content.startswith(command_prefix+"stop"):
            ...
            if isStarted:
                desc = "翻訳を終了します"
                embed = create_command_msg("stopped", desc, botName, botIcon)
                await message.channel.send(embed=embed)
        ...
        if message.content.startswith(command_prefix+"set"):
            ...
            if isStarted:
                ...
                if len(args) == 0:
                    ...
                    embed = create_command_msg("set", desc, botName, botIcon)
                    await message.channel.send(embed=embed)
                ...
                if len(args) > 10:
                    ...
                    embed = create_command_msg("error", desc, botName, botIcon)
                    await message.channel.send(embed=embed)
        ...
        if message.content.startswith(command_prefix+"help"):
            ...
            embed = create_command_msg("help", desc, botName, botIcon)
            await message.channel.send(embed=embed)
